    ...
    const renderStep1 = () => (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
            <div className="relative group">
                <Building2 className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${errors.organizationName ? 'text-red-500' : 'text-[#FF5C8A]'}`} size={14} />
                <input 
                    type="text" 
            ...
            <div className="relative group">
                <Users className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${errors.contactPerson ? 'text-red-500' : 'text-[#FF5C8A]'}`} size={14} />
                <input 
                    type="text" 
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="relative">
                    <Phone className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${errors.phoneNumber ? 'text-red-500' : 'text-[#FF5C8A]'}`} size={14} />
                    <input 
                        type="tel" 
                ...
                <div className="relative">
                    <Mail className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${errors.email ? 'text-red-500' : 'text-[#FF5C8A]'}`} size={14} />
                    <input 
                        type="email" 
            ...
            <div className="relative group">
                <ClipboardList className={`absolute left-3 top-3 transition-colors ${errors.address ? 'text-red-500' : 'text-[#FF5C8A]'}`} size={14} />
                <textarea 
                    rows="2" 
            ...
            <button 
                onClick={handleNext} 
                className="w-full py-4 bg-[#FF5C8A] text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-pink-100/50 transition-all hover:translate-y-[-2px] active:scale-[0.98] mt-4 flex items-center justify-center gap-2"
            >
                Next: Order Details
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            <div className="grid grid-cols-4 gap-1.5 px-0.5">
                {['corporate', 'school', 'wholesale', 'other'].map(type => (
                    <button key={type} onClick={() => setFormData({...formData, orderType: type})} className={`py-2.5 rounded-xl border text-[8px] font-black uppercase transition-all shadow-sm ${formData.orderType === type ? 'bg-[#FF5C8A] text-white border-[#FF5C8A] shadow-pink-100' : 'bg-white text-gray-400 border-gray-100 h-full flex items-center justify-center'}`}>{type}</button>
                ))}
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setFormData({...formData, fabricPreference: 'platform-provided'})} className={`p-3 rounded-2xl border text-left transition-all ${formData.fabricPreference === 'platform-provided' ? 'border-[#FF5C8A] bg-[#FF5C8A]/5 ring-1 ring-[#FF5C8A]/20' : 'bg-white border-gray-100'}`}>
                        <p className={`text-[10px] font-black uppercase tracking-tight ${formData.fabricPreference === 'platform-provided' ? 'text-[#FF5C8A]' : 'text-gray-900'}`}>Silaiwala Premium</p>
                        <p className="text-[8px] text-gray-400 font-medium leading-tight mt-0.5">We source high quality material</p>
                    </button>
                    <button onClick={() => setFormData({...formData, fabricPreference: 'customer-provided'})} className={`p-3 rounded-2xl border text-left transition-all ${formData.fabricPreference === 'customer-provided' ? 'border-[#FF5C8A] bg-[#FF5C8A]/5 ring-1 ring-[#FF5C8A]/20' : 'bg-white border-gray-100'}`}>
                        <p className={`text-[10px] font-black uppercase tracking-tight ${formData.fabricPreference === 'customer-provided' ? 'text-[#FF5C8A]' : 'text-gray-900'}`}>Self-Provided</p>
            ...
            <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                    {[
                        ...
                        <button key={opt.id} onClick={() => setFormData({...formData, measurementMethod: opt.id})} className={`px-4 py-3 rounded-2xl border flex items-center justify-between transition-all ${formData.measurementMethod === opt.id ? 'border-[#FF5C8A] bg-[#FF5C8A]/5 ring-1 ring-[#FF5C8A]/20' : 'bg-white border-gray-100'}`}>
                            <div className="flex items-center gap-4">
                                <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${formData.measurementMethod === opt.id ? 'bg-[#FF5C8A] border-[#FF5C8A] text-white' : 'border-gray-200 text-transparent'}`}><Check size={10} strokeWidth={4} /></div>
                                <div className="text-left">
                                    <p className={`text-[10px] font-black uppercase tracking-tight ${formData.measurementMethod === opt.id ? 'text-[#FF5C8A]' : 'text-gray-800'}`}>{opt.label}</p>
            ...
            <AnimatePresence mode="wait">
                {formData.measurementMethod === 'standard-sizes' && (
                    ...
                    >
                        <p className="text-[9px] font-black text-[#FF5C8A] uppercase italic mb-3 text-center tracking-[0.2em]">Breakdown Required</p>
                            ...
                            {['S', 'M', 'L', 'XL', 'XXL'].map(size => (
                                <div key={size} className="text-center space-y-1.5">
                                    ...
                                    <input 
                                        type="number"
                                        min="0"
                                        className="w-full py-2 bg-white border border-gray-100 rounded-xl text-center text-[11px] font-black outline-none focus:border-[#FF5C8A] focus:ring-1 focus:ring-pink-100"
            ...
            <div className="flex gap-3 pt-3">
                ...
                <button 
                    ...
                    className="flex-[2] py-4 bg-[#FF5C8A] text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-xl shadow-pink-100 transition-all hover:translate-y-[-2px] active:scale-[0.98] disabled:opacity-50"
                >
                    ...
                    {steps.map((step, idx) => (
                        <React.Fragment key={step.id}>
                            <div className="flex flex-col items-center gap-1.5 relative">
                                <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${currentStep >= step.id ? 'bg-[#FF5C8A] text-white shadow-lg' : 'bg-white text-gray-200 border border-gray-50'}`}>
                                    {React.cloneElement(step.icon, { size: 14 })}
                                </div>
                                <span className={`text-[7px] font-black uppercase absolute -bottom-4 whitespace-nowrap ${currentStep >= step.id ? 'text-[#FF5C8A]' : 'text-gray-300'}`}>{step.title}</span>
                            </div>
                            {idx < steps.length - 1 && <div className={`flex-1 h-[1.5px] mx-1 rounded-full ${currentStep > step.id ? 'bg-[#FF5C8A]' : 'bg-gray-100'}`} />}
                        </React.Fragment>
                    ))}
                </div>
